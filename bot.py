...
def getMenu(update: Update, context: CallbackContext): # risponde a /menu <msg>, fornisce il menu del giorno indicato (vedi menu.py)
    ...
    if week == "":
        update.message.reply_text("Il menù invernale non è ancora disponibile, se hai una foto del menù puoi inviarla a [Giorgio](tg://user?id=" + str(private.adminID) + ")", parse_mode=PARSEMODE_MARKDOWN_V2)
    else:
        if day[1] == -1:
